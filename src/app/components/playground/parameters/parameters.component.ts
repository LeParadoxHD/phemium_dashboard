...
export class ParametersComponent implements OnInit {
  ...
  constructor(private _fb: FormBuilder, private _store: Store, private injector: Injector) {}
    ...
    runInInjectionContext(this.injector, () => {
      ...
      for (const param of this.api.params) {
        if (param.type.length > 1) {
          ...
          this.multipleTypes
            .get(param.name)
            .setValue(values?.[param.name] || param.type[0], { emitEvent: false });
        }
      }
        ...
        .pipe(
          ...
          switchMap((parameters) =>
            this._store.dispatch(
              new ViewActions.UpdateViewVirtualParameters(this.api.id, parameters)
            )
          ),
          takeUntilDestroyed()
        )
  ...
  send() {
    ...
    this._store.dispatch(
      new ViewActions.PerformRequest(this.api.group.name, this.api.name, parameters)
    );
  }
}
